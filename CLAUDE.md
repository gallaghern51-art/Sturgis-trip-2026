...
- **Highway shields**: `netlify/functions/shield.mjs` resolves a route key (`MT-84`) to real Wikimedia Commons artwork and proxies it with a 1-year immutable CDN cache. Commons names files inconsistently per state (`I-90.svg`, `US 20.svg`, `MT-84.svg`, `SD 87.svg`, `California 1.svg`), so it tries the five conventions in one batched title lookup and falls back to Commons search — measured 25/25 across 20 states. **There is deliberately no bundled artwork and no route→filename map**: that list is unbounded and never finished. `RoadShield.jsx` falls back to a text chip on 404 (and under plain `vite dev`, where the function does not run).
