...
- **Highway shields**: every shield is downloaded. `netlify/functions/shield.mjs` resolves a route key (`MT-84`) to real Wikimedia Commons artwork and proxies it with a 1-year immutable CDN + browser cache, so a route is fetched once and is offline thereafter. Commons names files inconsistently per state (`I-90.svg`, `US 20.svg`, `MT-84.svg`, `SD 87.svg`, `California 1.svg`), so it tries five conventions in one batched title lookup and falls back to Commons search — measured 25/25 across 20 states. **No bundled artwork, no route→filename map, and no drawn fallback**: when Commons genuinely has nothing, `RoadShield.jsx` sets the route number as plain type rather than faking a sign. `vite.config.js` serves `/.netlify/functions/*` in dev via `netlifyFunctionsInDev()`, so `npm run dev` behaves like production — do not reintroduce workarounds premised on functions being absent locally.
