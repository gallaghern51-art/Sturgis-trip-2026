...
- **Default basemap is hybrid satellite** (Esri imagery z19 + roads + place labels). All basemap styles live in `src/engine/basemaps.js`, shared by MapView and RideMode — now four styles (sat / streets=liberty / dark / light) plus `ensureTerrain(map, on)`: 3D terrain + hillshade from AWS/Mapzen terrarium DEM tiles (free, no key). MapView has a 3D toggle; `ensureTerrain` is idempotent and re-asserted from `drawAll` so it survives `setStyle`.
