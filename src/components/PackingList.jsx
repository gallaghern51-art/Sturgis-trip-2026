...
const LIST = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'cash', en: 'Cash, small bills — rally vendors and rural fuel', es: 'Efectivo en billetes chicos — vendedores del rally y gasolina rural' },
...
const TOTAL = LIST.reduce((a, s) => a + s.items.length, 0);
...
export default function PackingList({ onClose }) {
  ...
  useEffect(() => {
    try { localStorage.setItem(KEY, JSON.stringify(done)); } catch { /* full */ }
  }, [done]);
      ...
      <div className="modal packing" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          {LIST.map((sec) => (
            <div key={sec.id} className="pack-sec">
              <h4>{name(sec)} <span className="cnt">{sec.items.filter((i) => done[i.id]).length}/{sec.items.length}</span></h4>
              {sec.items.map((item) => (
                <label key={item.id} className={`pack-item${done[item.id] ? ' done' : ''}`}>
        ...
        <div className="modal-foot">
          <button
            ...
            onClick={() => { if (confirm(lang === 'es' ? '¿Desmarcar todo?' : 'Uncheck everything?')) setDone({}); }}
