...
import { conditionKind } from '../engine/conditions.js';
...
/**
 * One weather glyph, shared by the day panel and the nav HUD so the two never
 * drift apart — the rider should see the same mark for the same sky.
 */
export default function WeatherIcon({ code, className = '' }) {
  return (
    <svg viewBox="0 0 24 26" className={className} aria-hidden="true">
      {ICONS[conditionKind(code)] ?? ICONS.unknown}
    </svg>
  );
}
